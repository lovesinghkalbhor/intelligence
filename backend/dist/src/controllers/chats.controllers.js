...
exports.handleChatMessage = exports.newMessage = void 0;
...
const newMessage = async (title) => {
    const data = {
        model: "meta-llama/llama-4-scout-17b-16e-instruct",
        messages: [
            {
                role: "user",
                content: `Your taks is to create a title out of this sentence or word give me short and concise and only give me one title.
    ...
    const response = await axios_1.default.post("https://api.groq.com/openai/v1/chat/completions", data, {
        headers: {
...
const handleChatMessage = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (threadId) {
        thread = await threads_1.default.findById(threadId);
        ...
        thread = new threads_1.default({ title: await newMessage(message) }); // Use first message as title
    }
    ...
    const groqMessages = thread.messages.map((msg) => {
        ...
        return {
            role: msg.role,
            content: (0, helperFunctions_1.formatContentForGroq)(msg.content),
        };
    });
    ...
    const llmResponse = await axios_1.default.post("https://api.groq.com/openai/v1/chat/completions", llmRequestData, {
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${process.env.AI_API_KEY}`,
        },
